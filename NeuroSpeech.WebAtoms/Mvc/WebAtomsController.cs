...
namespace NeuroSpeech.WebAtoms.Mvc
{
    ...
    public abstract class SecureRepositoryFactory
    {
        public abstract T New<T>();

        public static SecureRepositoryFactory Factory { get; set; }
    }
		...
		where TOC : ISecureRepository
	{
        ...
        protected override void Initialize(System.Web.Routing.RequestContext requestContext)
        {
            ...
            requestContext.HttpContext.Items["Repository"] = this.ObjectContext;
        }
		...
		protected internal T FormValue<T>(string name, bool optional = false, T defValue = default(T)) {
			...
			if (!FormModel.TryGetValue(name, out val)) { 
				if(!optional)
					throw new AtomValidationException(name + " is required");
				return defValue;
			}
            if (val == null)
                return defValue;
		...
		private void ParseDates(Dictionary<string, object> data)
		{
			foreach (var item in data.ToList())
			{
                ...
                if (val.StartsWith("/DateISO(") && val.EndsWith(")/"))
                {
                    data[item.Key] = WebAtomsMvcHelper.ToDateTime(val);
                }
			}
		}
            ...
            where T : class
        {
            ...
            return new AtomQueryableResult<T>(ObjectContext.Where<T>(filter), ObjectContext);
        }
