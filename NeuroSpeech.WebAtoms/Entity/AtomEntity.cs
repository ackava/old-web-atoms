        ...
        void IJavaScriptDeserializable.Deserialize(System.Collections.Generic.Dictionary<string, object> values)
        {
            foreach (PropertyDescriptor pd in TypeDescriptor.GetProperties(this))
            {
                ...
                if (val != null)
                {
                    ...
                    {
                        if (typeof(AtomEntity).IsAssignableFrom(pt))
                            continue;
                        var c = pt.GetConstructors().FirstOrDefault(x => x.GetParameters() == null || x.GetParameters().Length == 0);
                        if (c != null)
                        {
                            var co = Activator.CreateInstance(pt);
                            DictionaryModelBinder.Bind(co , val as Dictionary<string,object>);
                            pd.SetValue(this,co);
                        }
                        continue;
                    }
