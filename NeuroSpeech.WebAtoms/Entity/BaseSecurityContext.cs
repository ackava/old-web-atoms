...
namespace NeuroSpeech.WebAtoms.Entity
{
    ...
    public abstract class BaseSecurityContext
    {
        ...
        internal void ValidateBeforeSave(ISecureRepository db, ChangeSet cs)
        {
            if (IgnoreSecurity)
                return;
            foreach (Change item in cs.UpdatedEntities)
            {
                var entity = item.Entity;
                var type = item.Entity.GetType();
                GenericMethods.InvokeGeneric(this, "VerifyEntityModify", type, entity, item.ModifiedProperties);
            }
            foreach (Change item in cs.Deleted)
            {
                var entity = item.Entity;
                var type = item.Entity.GetType();
